/* /aff-wrap.skillbox.js
 * «Хитроссылки» — клик по [data-cta-url] с доменом skillbox.ru
 * оборачивает в трекинг go.2038.pro и открывает в новой вкладке.
...
(function () {
    ...
    // === НАСТРОЙКИ ===
    var WRAP_BASE = 'https://go.2038.pro/73b6f9de563a3b81';
    var ERID = '2VfnxvfxmGL';
    var M_PARAM = '2';
    var MATCH_HOST = 'skillbox.ru';

    // === УТИЛИТЫ ===

    function isSkillboxDomain(hostname) {
    ...
    document.addEventListener('click', function (e) {
        ...
        try {
            ...
            if (!isSkillboxDomain(u.hostname)) return; // не наш — пропускаем
        } catch (err) {
            return;
        }
    ...
    // === SAFETY: оборачиваем случайные <a> на skillbox.ru (если проскочат) ===

    function wrapStrayAnchors(root) {
        var anchors = (root || document).querySelectorAll('a[href]');
        for (var i = 0; i < anchors.length; i++) {
            var a = anchors[i];
            if (a.hasAttribute('data-aff-skillbox')) continue;
